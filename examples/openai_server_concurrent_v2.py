#!/usr/bin/env python3
"""
OpenAI-compatible TTS API server (v2 热加载版) for faster-qwen3-tts.

与 openai_server_concurrent.py 的区别：
  - 音色数据来源从 voices.json 切换为 MySQL + Redis（通过 config.json 配置）。
  - 音色管理（增删改查）由独立的 voice_manager_api.py 负责，本服务只读。
  - 无需重启即可使用新音色：管理 API 写库 + 清缓存后，下一次请求自动拉取。
...
tts_model = None
tts_models = []
voice_db: Optional[VoiceDB] = None
...
def resolve_voice(voice_id: str) -> dict:
    """通过 Redis + MySQL 获取 voice 配置；未找到则 400。"""
    if voice_db is None:
        raise HTTPException(status_code=503, detail="VoiceDB not initialized")
    cfg = voice_db.get_voice(voice_id)
    if cfg:
        return cfg
    raise HTTPException(
        status_code=400,
...
def _prime_voice_caches(
    ...
    *,
    ...
    """预热：把参考音频编码进 _voice_prompt_cache，避免首个真实请求的 TTFA 被拖慢。

    voice_list 来自 VoiceDB.list_voices()，每条形如
    {"voice_id": ..., "ref_audio": ..., "ref_text": ..., "language": ...}。
    """
    if not voice_list:
...
def _parse_args():
    p = argparse.ArgumentParser(
        description="OpenAI-compatible TTS server (v2 热加载版) for faster-qwen3-tts",
    ...
    p.add_argument(
        ...
        help="配置文件路径（MySQL/Redis/目录等），默认 ../config.json",
    )
    p.add_argument(
        "--model",
...
def main():
    global tts_model, tts_models, voice_db, SAMPLE_RATE, _request_sem, _metrics_enabled

    args = _parse_args()

    config = load_config(args.config)
    voice_db = VoiceDB(config)

    conn_status = voice_db.check_connections()
    logger.info("连接检测: MySQL=%s Redis=%s", conn_status["mysql"], conn_status["redis"])
    if not conn_status["mysql"]:
        logger.error("MySQL 不可用，退出")
        sys.exit(1)

    active_voices = voice_db.list_voices(status="active")
    logger.info("从 MySQL 加载到 %d 个 active 音色", len(active_voices))
