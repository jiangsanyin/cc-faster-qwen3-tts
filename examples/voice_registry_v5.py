"""
v5 音色注册表：JSON 文件 + 文件锁 + 原子替换写入。

与 v4 功能一致，为 v5 服务提供独立的音色注册表管理，避免版本间依赖。
供 voice_manager_router_v5.py（读写）与 openai_server_v5.py（热加载只读）使用。
"""
...
_warned_bad_entries: set = set()


def _iter_valid_entries(data: Dict[str, Any]) -> Iterator[Tuple[str, dict]]:
    """
    遍历注册表数据，仅产出合法的 (voice_id, entry-dict) 二元组。

    对非 dict 条目：
      - 键以 '_' 开头的（如 "_comment" 注释键）静默跳过；
      - 其他非法条目打印一次 WARNING 后跳过，避免在调度/热加载循环中刷屏。
    """
    ...
    for vid, entry in data.items():
        if isinstance(entry, dict):
            yield vid, entry
            continue
        if isinstance(vid, str) and vid.startswith("_"):
            continue
        key = (str(vid), type(entry).__name__)
        if key not in _warned_bad_entries:
            _warned_bad_entries.add(key)
            _logger.warning(
                "voice_registry: skip invalid entry vid=%r type=%s (expected dict); "
                "please fix voices_registry_v5.json",
                vid, type(entry).__name__,
            )


def _as_entry_dict(entry: Any) -> Optional[dict]:
    """将单个条目校验为 dict，非法则返回 None。"""
    return entry if isinstance(entry, dict) else None


def load_config_v5(config_path: str) -> dict:
    """
    加载 v5 配置文件（JSON）。
...
class VoiceRegistryV5:
    ...
    def get_voice_active(self, voice_id: str) -> Optional[dict]:
        ...
        """
        data = self.read_all()
        entry = _as_entry_dict(data.get(voice_id))
        if not entry:
            return None
    ...
    def get_entry_raw(self, voice_id: str) -> Optional[dict]:
        ...
        """
        data = self.read_all()
        e = _as_entry_dict(data.get(voice_id))
    ...
    def list_voices(self, status: str = "active") -> List[dict]:
        ...
        data = self.read_all()
        out: List[dict] = []
        for vid, entry in _iter_valid_entries(data):
...
class HotVoiceRegistryV5:
    ...
    def count_active(self) -> int:
        ...
        self._reload_if_needed()
        n = 0
        for _vid, entry in _iter_valid_entries(self._cache):
            if entry.get("status", "active") != "disabled":
                n += 1
        return n
    ...
    def resolve_active_voice(self, voice_id: str) -> Optional[dict]:
        ...
        """
        self._reload_if_needed()
        entry = _as_entry_dict(self._cache.get(voice_id))
        if not entry:
            return None
        if entry.get("status", "active") == "disabled":
            return None
    ...
    def list_active_voice_cfgs(self) -> List[dict]:
        ...
        self._reload_if_needed()
        out: List[dict] = []
        for vid, entry in _iter_valid_entries(self._cache):
            if entry.get("status", "active") == "disabled":
                continue
            out.append(_entry_to_voice_cfg(vid, entry))
        return out
...
def valid_voice_prompt_cache_keys(registry: Dict[str, Any]) -> set:
    ...
    """
    S = set()
    for _vid, entry in _iter_valid_entries(registry):
        if entry.get("status", "active") == "disabled":
            continue
...
def maybe_cleanup_voice_prompt_cache(
    ...
    *,
    ...
    n_reg = sum(
        1
        for _v, e in _iter_valid_entries(registry)
